...
bool DateTime::_Time::compare_local_Time(std::shared_ptr<_Time> _DT) {
    ...
    ptime local_DateTime(dt, hours(ltm->tm_hour) + minutes(ltm->tm_min) + seconds(ltm->tm_sec));
    ptime that_DateTime(dt, hours(_DT->get_Hour()) + minutes(_DT->get_Minute()) + seconds(_DT->get_Second()));

    if (that_DateTime < local_DateTime)
        return false;
}

bool DateTime::_Time::compare_Time(std::shared_ptr<_Time> _DT) {
    using namespace boost::posix_time;
    using namespace boost::gregorian;

    date dt(1900, Feb, 01);

    ptime this_Time(dt, hours(this->m_Hour) + minutes(this->m_Minute) + seconds(this->m_Sec));
    ptime that_Time(dt, hours(_DT->get_Hour()) + minutes(_DT->get_Minute()) + seconds(_DT->get_Second()));
...
bool DateTime::_Date::compare_local_Date(std::shared_ptr<_Date> _DT) {
    using namespace boost::posix_time;
    using namespace boost::gregorian;
    time_t now = time(0);
    tm* ltm = localtime(&now);
...
bool DateTime::_Date::compare_Date(std::shared_ptr<_Date> _DT) {
    using namespace boost::posix_time;
    using namespace boost::gregorian;
...
bool DateTime::_DateTime::compare_local_DateTime(std::shared_ptr<_DateTime>  _DT) {
